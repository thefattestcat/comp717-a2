...
class Takeaway extends Component {
  ...
  newGame = () => {
    ...
    this.setState( {
      ...
      positionState: p,
      winner: 0,
      numberOfChips: this.state.numberOfChips,
      minmaxLevel: 0,
   ...
   */
  setSquareState = async (squareId, playerId, amount) => {
    ...
    for(var i = 0; i < 2; i++) {
      ...
      sqs.push(
        <GridSquare 
          ...
          player={0}
          ref={this.gs[i]}
          amount={this.state.numberOfChips}
  ...
  render() {
    let slider = <RangeSlider value={this.state.takeaway} onChange={changeEvent => this.setState({takeaway: changeEvent.target.value})} min={1} max={3} tooltipStyle={{display: 'none'}}/>;
    return (
      <>
      <Container>
        <Row style={{ paddingTop: '5rem'}}>
          <Col md="4" xs="12">
            ...
            <div>
              <p>Takeaway amount: {this.state.takeaway}</p>
              {
                slider
              }
            </div>
